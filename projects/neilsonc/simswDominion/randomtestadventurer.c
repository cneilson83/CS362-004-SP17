...
int main(){
	...
	int seed = 1000, choice1, choice2, choice3, handpos, bonus, result, temphand[MAX_HAND];
	int handBefore = 0, handAfter = 0;
	...
	for(i = 0; i < NUM_TESTS; i++) {
		...
		handBefore = G.handCount[thisPlayer];
		
		//fill player hand with random cards
		for(j = 0; j < handBefore; j++) {
			randC = rand() % 13;
			G.hand[thisPlayer][j] = cards[randC];
		}
		coinsBefore = updateCoins(thisPlayer, &G, 0);
		
		// copy game state, call cardEffect with Adventurer, 
		// and verify the return value 
		memcpy(&test, &G, sizeof(struct gameState));
		test.handCount[thisPlayer] = handBefore;
		playAdventurer(&test, thisPlayer, 0, 0, temphand, 0);
		newCards = 2;
		discardedCard = 1;
		...
		if(test.handCount[thisPlayer] == handBefore + newCards - discardedCard)
		...
		// test that card was discarded properly by adding it to played pile
		if(test.playedCardCount == G.playedCardCount + discardedCard)
			printf("PASSED test for discard count\n");
		else
			printf("FAILED discard count <iteration #%i>\n", i + 1);
